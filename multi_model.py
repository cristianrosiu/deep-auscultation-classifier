...
def multi_model_custom(input_shape, drop_rate, test_metrics):
    ...
    model.compile(loss=['binary_crossentropy', 'categorical_crossentropy', 'mse'], 
                  optimizer=Adam(clipnorm= 1., lr=0.0001), metrics=test_metrics)
...
def multi_model(input_shape, drop_rate, test_metrics):
    ...
    model.compile(loss=['binary_crossentropy', 'categorical_crossentropy'], optimizer=Adam(lr=0.0001), metrics=test_metrics, loss_weights=[0.6, 0.4])
...
if __name__ == '__main__':
    ...
    config = Config(epoch=100, batch=32, n_mfcc=20, num_seg=5, sr=None)
    ...
    print(classification_report(y1_test, model.predict_classes(X_test)))